    ...
    worker: async (summary) => {
      const detailResponse = await retryTask({
        ...
        task: async () => {
          ...
          assertSuccessfulResponse(response, `Claude conversation ${summary.id}`);
          return response;
        },
      });

      return createConversationItem(summary, detailResponse, orgId, rawSink, contentDir);
    },
  });
...
async function syncClaudeChatsFromChromeTab(
  ...
  emitProgress(
    options.onProgress,
    "bootstrap",
    `Finding the active ${browserId} Claude tab`,
  );
  const tab = await findChromiumTab(browserId, [CLAUDE_HOST]);

  if (!tab) {
    throw new Error(`No open ${browserId} tab for Claude was found.`);
  }

  emitProgress(
    options.onProgress,
    "bootstrap",
    `Discovering Claude organization from the active ${browserId} tab`,
  );
...
async function discoverOrganizationId(page: Page): Promise<string> {
  ...
  throw new Error(
    "Could not determine the active Claude organization id from the live browser session.",
  );
}
...
async function discoverOrganizationIdFromChromeTab(tab: GoogleChromeTabTarget): Promise<string> {
  ...
  throw new Error(
    `Could not determine the active Claude organization id from the active ${browserLabel} tab.`,
  );
}
...
function extractOrganizationIdFromPayload(payload: unknown): string | null {
  for (const record of collectRecords(payload)) {
    const candidate =
      ...
      readString(record, "organization_uuid") ??
...
async function fetchConversationSummaries(
  ...
  while (hasMore && (requestedLimit === undefined || summaries.length < requestedLimit)) {
    emitProgress(
      onProgress,
      "page",
      `Fetching Claude conversations page ${pageNumber}`,
      summaries.length,
    );
    const response = await fetchClaudeJson(
      page,
    ...
    emitProgress(
      onProgress,
      "page",
      `Fetched Claude conversations page ${pageNumber}`,
      summaries.length,
    );
    pageNumber += 1;
  }
...
async function fetchClaudeJsonFromChromeTab(
  tab: GoogleChromeTabTarget,
  pathValue: string,
...
function toConversationSummary(
  ...
  const title =
    firstString(record, ["name", "title", "chat_name"]) ?? `Claude chat ${id.slice(0, 8)}`;
  const updatedAt =
    firstString(record, ["updated_at", "created_at", "last_message_at", "last_activity_at"]) ??
...
function parseConversationDetail(payload: unknown): ClaudeConversationDetail {
  ...
  return {
    ...
    updatedAt: toIsoString(
      firstString(record, ["updated_at", "created_at"]) ?? new Date().toISOString(),
    ),
    ...(model ? { model } : {}),
    ...(summary ? { summary } : {}),
...
async function buildConversationItems({
  ...
  worker(summary: ClaudeConversationSummary): Promise<TroveItem>;
...
function createConversationItem(
  ...
  detailResponse: ClaudeFetchResponse | GoogleChromeFetchResponse,
  orgId: string,
  rawSink: ReturnType<typeof createJsonlSink>,
  contentDir: string,
): TroveItem {
...
function toClaudeMessage(record: Record<string, unknown> | null): ClaudeMessage | null {
  ...
  return {
    ...
    ...(readString(record, "created_at")
      ? { createdAt: toIsoString(readString(record, "created_at")!) }
      : {}),
    ...(readString(record, "updated_at")
      ? { updatedAt: toIsoString(readString(record, "updated_at")!) }
      : {}),
...
function deriveKnowledgeExcerpt(value: string): string {
  ...
  const meaningful = paragraphs.find(
    (part) => !/^(skip to content|navigation menu|sign in|appearance settings)$/i.test(part),
  );
...
async function collectHybridSummaries({
  ...
  backfillSummaries: ClaudeConversationSummary[];
  recentSummaries: ClaudeConversationSummary[];
}> {
  if (limit === undefined) {
    return {
...
function mergeSummaries(
  ...
  const merged: ClaudeConversationSummary[] = [];
  const seenIds = new Set<string>();
