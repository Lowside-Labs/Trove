...
const INSTAGRAM_BLOKS_VERSION_ID =
  "f0fd53409d7667526e529854656fe20159af8b76db89f40c333e593b51a2ce10";
...
export async function syncInstagramSaved(
  ...
  const collectionsById = await fetchSavedCollections(requestContext).catch(
  ...
  while (true) {
    ...
    emitProgress(
      options.onProgress,
      "page",
      `Fetched Instagram saved page ${pageNumber}`,
      items.length,
    );
...
async function fetchInstagramUsername(cookieHeader: string): Promise<string> {
  const response = await fetch(
    new URL("/api/v1/accounts/edit/web_form_data/", INSTAGRAM_BASE_URL),
    {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Instagram account request failed with ${response.status}: ${body.slice(0, 400)}`,
    );
  }
  ...
  if (!username) {
    throw new Error(
      "Could not determine the authenticated Instagram username from the web account settings response.",
    );
  }
...
async function fetchSavedCollections(
  ...
  const response = await fetch(new URL("/graphql/query", INSTAGRAM_BASE_URL), {
    headers: {
      ...buildInstagramHeaders(
        ...
        `${INSTAGRAM_BASE_URL}/${context.username}/saved/`,
      ),
  ...
  if (!response.ok) {
    const body = await response.text();
    throw new Error(
      `Instagram saved collections request failed with ${response.status}: ${body.slice(0, 400)}`,
    );
  }
...
async function fetchSavedPostsPage(
  ...
  const response = await fetch(url, {
    headers: buildInstagramHeaders(
      context.cookieHeader,
      `${INSTAGRAM_BASE_URL}/${context.username}/saved/all-posts/`,
    ),
    method: "GET",
  });
  ...
  if (!response.ok) {
    const body = await response.text();
    throw new Error(
      `Instagram saved posts request failed with ${response.status}: ${body.slice(0, 400)}`,
    );
  }
...
function parseSavedCollectionsPayload(
  payload: InstagramSavedCollectionsResponse,
): Map<string, string> {
...
function parseSavedPostsPayload(
  payload: unknown,
  collectionsById: Map<string, string>,
): ParsedSavedPostsPage {
  ...
  return cleanText(data?.next_max_id)
    ? { items, rawItems, nextPageCursor: cleanText(data?.next_max_id) as string }
    : { items, rawItems };
}

function normalizeSavedMedia(
  media: InstagramSavedMedia | undefined,
  collectionsById: Map<string, string>,
): TroveItem | null {
  ...
  const url = buildInstagramMediaUrl(
    code,
    cleanText(media?.product_type),
    cleanText(media?.subtype_name_for_REST__),
  );
  ...
  const collectionNames = collectionIds
    .map((value) => collectionsById.get(value))
    .filter((value): value is string => Boolean(value));
  const savedAt = normalizeSavedTimestamp(media);
  const productType =
    cleanText(media?.product_type) ?? cleanText(media?.subtype_name_for_REST__) ?? "post";
...
function buildSavedRawRecord(
  ...
  return {
    ...
    takenAt: typeof media?.taken_at === "number" ? normalizeUnixSeconds(media.taken_at) : null,
    captionCreatedAt:
      typeof media?.caption?.created_at_utc === "number"
        ? normalizeUnixSeconds(media.caption.created_at_utc)
        : null,
    savedAtSource: inferSavedTimestampSource(media),
    productType,
    mediaType: typeof media?.media_type === "number" ? media.media_type : null,
