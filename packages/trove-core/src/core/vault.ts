...
function renderIndexMarkdown(snapshot: {
  ...
  const contentCount = snapshot.items.filter(
  ...
  for (const sourceId of listSyncSourceIds()) {
    ...
    const suffix = sourceStats?.lastSyncedAt
      ? `, last synced ${sourceStats.lastSyncedAt.slice(0, 10)}`
      : "";
  ...
  if (recentItems.length === 0) {
    ...
    for (const item of recentItems) {
      ...
      lines.push(
        `- [${item.savedAt.slice(0, 10)}] ${item.source}/${kind} [${escapeMarkdown(item.title)}](${item.url})${author}`,
      );
    }
  }
