...
const browserDefinitions: Record<
  SupportedBrowserId,
  BrowserDefinition & { keychain?: BrowserKeychainTarget }
> = {
  chrome: {
    id: "chrome",
    name: "Google Chrome",
  ...
  brave: {
    ...
    notes:
      "Chromium profile layout is known, but Safe Storage naming is not yet verified in Trove.",
  },
  arc: {
    id: "arc",
    name: "Arc",
...
export function resolveChromiumBrowser(
  ...
  if (!fs.existsSync(cookiesPath)) {
    throw new Error(
      `Cookies file was not found for ${browser.name} profile "${resolvedProfile}" at ${cookiesPath}.`,
    );
  }
...
async function deriveMacosCookieKey(target: BrowserKeychainTarget): Promise<Buffer> {
  ...
  if (!password) {
    throw new Error(
      `Could not read ${target.service} from macOS Keychain for account ${target.account}.`,
    );
  }
...
function loadCookiesFromStore(
  cookiesPath: string,
  domains: string[],
  decryptionKey: Buffer,
): BrowserSession["playwrightCookies"] {
...
function mapCookieRow(
  ...
  const value =
    row.value || decryptCookieValue(row.encrypted_value, decryptionKey, hasHostKeyPrefix);
...
function decryptCookieValue(
  ...
  const unpadded =
    paddingLength > 0 ? decrypted.subarray(0, decrypted.length - paddingLength) : decrypted;
