...
export class TaskDashboardRenderer {
  ...
  completeRun(label: string, itemCount: number): void {
    ...
    this.logPlainLine(
      `${label}: imported ${itemCount} item${itemCount === 1 ? "" : "s"} in ${formatElapsed(run)}`,
    );
    this.render();
  ...
  private getOrderedRuns(): ProgressRunSnapshot[] {
    return this.order
      .map((label) => this.runs.get(label))
      .filter((run): run is ProgressRunSnapshot => Boolean(run));
  }

  private logPlainProgress(
    label: string,
