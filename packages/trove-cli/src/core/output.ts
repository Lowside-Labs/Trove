...
export class TerminalOutput {
  ...
  get columns(): number {
    return typeof this.stdout.columns === "number" && this.stdout.columns > 0
      ? this.stdout.columns
      : 100;
  }
...
export function renderInitReport(
  ...
  output.line(
    output.toned(
      `From inside this folder, \`trove\` commands target this workspace automatically.`,
      "muted",
    ),
  );
  output.line(output.toned("Trove will remember this workspace for future commands.", "muted"));
  output.line(
    output.toned(`Next: run \`${syncCommand}\`, then open the folder in your agent.`, "muted"),
  );
  ...
  if (isDefaultTroveRoot(paths.root)) {
    output.line(
      output.toned(
        "Tip: a visible folder such as `~/Trove` is easier to open in Claude Code or Obsidian than `~/.trove`.",
        "muted",
      ),
    );
  }
...
export function renderSearchResults(
  ...
  for (const [index, result] of results.entries()) {
    ...
    output.line(
      `${output.badge(result.source.toUpperCase(), tone)} ${output.strong(`${index + 1}.`)} ${title}`,
    );
...
export function renderStatsReport(output: TerminalOutput, report: StatsReport): void {
  ...
  if (report.lastSyncedAt) {
    output.line(
      output.toned(`Latest tracked sync ${formatRelativeTime(report.lastSyncedAt)}.`, "muted"),
    );
  }
...
export function renderCommandRunReports(
  output: TerminalOutput,
  source: string,
  runs: CommandRunReport[],
): void {
  ...
  output.line(
    output.toned(
      'Next: open `INDEX.md`, `AGENTS.md`, or `CLAUDE.md`, or use `trove search "<query>"`.',
      "muted",
    ),
  );
}
