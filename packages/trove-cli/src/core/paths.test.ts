...
describe("path helpers", () => {
  ...
  it("resolves init workspace options", () => {
    ...
    expect(resolveWorkspaceRoot({ path: "../vault", cwd: "/tmp/demo/app" })).toBe(
      "/tmp/demo/vault",
    );
  });
  ...
  it("rejects conflicting workspace flags", () => {
    expect(() => resolveWorkspaceRoot({ here: true, path: "/tmp/trove" })).toThrow(
      "Choose either --here or --path, not both.",
    );
  });
});
