...
describe("install.sh", () => {
  it("installs trove into a user-owned location and links the trove binary", () => {
    ...
    const pnpmLogPath = path.join(root, "pnpm.log");
    roots.push(root);
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "trove-fixture" }),
      "utf8",
    );

    createArchive(root, archivePath);
    writeExecutable(path.join(stubsDir, "node"), buildNodeStub("22"));
    writeExecutable(path.join(stubsDir, "pnpm"), buildPnpmStub());
    ...
    const pnpmLog = fs.readFileSync(pnpmLogPath, "utf8");
    expect(pnpmLog).toContain("install --frozen-lockfile --silent");
    expect(pnpmLog).toContain("run --silent build");
    expect(pnpmLog).toContain("prune --prod --silent");
  });
...
if [ "\${1-}" = "run" ] && [ "\${2-}" = "--silent" ] && [ "\${3-}" = "build" ]; then
  mkdir -p packages/trove-cli/dist
  cat > packages/trove-cli/dist/cli.js <<'EOF'
#!/usr/bin/env bash
printf 'trove stub\\n'
EOF
  chmod +x packages/trove-cli/dist/cli.js
  exit 0
fi
