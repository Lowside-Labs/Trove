...
import { buildHydratedContentRelativePath, renderMarkdownFrontmatter, writeHydratedMarkdown } from "./content.js";
...
export async function hydrateArchive(root: string | undefined, options: HydrateOptions): Promise<HydrateResult> {
  ...
  try {
    ...
    for (const [index, item] of candidates.entries()) {
      try {
        emitProgress(options.onProgress, "fetch", `Fetching ${item.title}`, index, candidates.length);
        const fetched = await fetchHtml(item.url);
        emitProgress(options.onProgress, "extract", `Extracting ${item.title}`, index, candidates.length);
        const document = extractReadableDocument(fetched.html, item.url);

        if (!document.content || document.content.trim().length === 0) {
          skippedCount += 1;
          emitProgress(options.onProgress, "persist", `Skipped ${item.title}`, index + 1, candidates.length);
          continue;
        }
        ...
        hydratedCount += 1;
        emitProgress(options.onProgress, "persist", `Hydrated ${item.title}`, index + 1, candidates.length);
      } catch {
        failedCount += 1;
        emitProgress(options.onProgress, "persist", `Failed ${item.title}`, index + 1, candidates.length);
      }
    }
...
function extractFallbackDocument(html: string, url: string): {
  title: string;
  author?: string;
  excerpt?: string;
  content: string;
