...
export function buildHydratedContentRelativePath(item: Pick<TroveItem, "source" | "title" | "externalId">): string {
  const hash = crypto.createHash("sha1").update(`${item.source}:${item.externalId}`).digest("hex").slice(0, 10);
...
export function renderMarkdownFrontmatter(fields: Record<string, string | string[] | undefined>): string {
  const lines = ["---"];
