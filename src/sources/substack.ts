...
async function fetchSavedPostsPage(cookieHeader: string, offset: number, limit?: number): Promise<unknown> {
...
function parseSavedPostsPayload(payload: unknown): ParsedSavedPage {
  ...
  for (const post of posts) {
    ...
    const publication = typeof post.publication_id === "number" ? publicationsById.get(post.publication_id) : undefined;
...
async function fetchSelfProfile(cookieHeader: string): Promise<SelfProfile> {
  ...
  if (!response.ok) {
    throw new Error(`Could not load Substack inbox page to resolve the authenticated user: ${response.status}`);
  }
...
async function fetchLikesPage(cookieHeader: string, userId: number, cursor?: string): Promise<unknown> {
...
function parseLikesPayload(payload: unknown, likesPageUrl: string): ParsedLikesPage {
  ...
  return data?.nextCursor
    ? { items, rawItems, nextCursor: data.nextCursor }
    : { items, rawItems };
}

function normalizeLikeItem(record: Record<string, unknown> | null, likesPageUrl: string): TroveItem | null {
  if (!record) {
    return null;
  }
...
function normalizeLikedPost(
  ...
  const excerpt = firstNonEmptyString(cleanText(post.subtitle), cleanText(post.description), cleanText(post.truncated_body_text));
...
function normalizeLikedComment(entityKey: string, likedAt: string, comment: Record<string, unknown>, likesPageUrl: string): TroveItem | null {
  ...
  return {
    ...
    content: body,
    ...(author ? { author } : {}),
    tags: publicationName ? ["substack", "like", "note", publicationName] : ["substack", "like", "note"],
    raw: {
      platform: "substack",
      kind: "like",
      entityKey,
...
function buildSavedRawRecord(
  ...
  return {
    ...
    savedAt: firstNonEmptyString(savedPost?.created_at, inboxItem?.saved_at, inboxItem?.inbox_date, post.post_date) ?? null,
    savedAtSource: savedPost?.created_at ? "savedPosts.created_at" : inboxItem?.saved_at ? "inboxItems.saved_at" : "post_date",
  ...
  return {
    ...
    post: post
      ? {
          ...
          postDate: cleanText(post.post_date),
          subtitle: cleanText(post.subtitle),
          author: cleanText(asRecord(Array.isArray(post.publishedBylines) ? post.publishedBylines[0] : null)?.name),
        }
      : null,
    comment: comment
      ? {
          ...
          url: cleanText(comment.handle) && cleanText(record.entity_key)
...
function normalizeSavedPost(
  ...
  const title = cleanText(post.title);
  const url = readPostUrl(post, publication);
  const savedAt = firstNonEmptyString(savedPost?.created_at, inboxItem?.saved_at, inboxItem?.inbox_date, post.post_date);
