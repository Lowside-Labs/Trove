...
function parseStarEntry(entry: ReturnType<typeof parse>): { item: TroveItem; raw: Record<string, unknown> } | null {
  ...
  const language = entry.querySelector('[itemprop="programmingLanguage"]')?.text.trim() ?? undefined;
